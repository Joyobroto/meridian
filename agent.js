...
//PATCHDED FOR JSON NORMALIZATION --START HERE--
function sanitizeJSONArgs(raw) {
    if (!raw || typeof raw !== "string") return {};
    try {
        return JSON.parse(raw);
    } catch {
        try {
            let repaired = jsonrepair(raw);
            // REGEX MAGIC: Mengambil hanya konten di dalam kurung kurawal {}
            // Ini yang bakal ngebuang "trailing characters" yang bikin error 400
            const match = repaired.match(/\{[\s\S]*\}/);
            if (match) repaired = match[0];
            
            return JSON.parse(repaired);
        } catch {
            log("warn", "sanitizeJSONArgs failed, defaulting to {}");
            return {};
        }
    }
}
//PATCHDED FOR JSON NORMALIZATION --END HERE--
...
const INTENT_PATTERNS = [{
        ...
        intent: "claim",
        re: /\b(claim|harvest|collect)\b.*\bfee/i
    }, {
        intent: "swap",
        re: /\b(swap|convert|sell|exchange)\b/i
    }, {
        intent: "selfupdate",
        ...
        intent: "balance",
        re: /\b(balance|wallet|sol|how much)\b/i
    }, {
        intent: "positions",
        re: /\b(position|portfolio|open|pnl|yield|range)\b/i
    }, {
        intent: "strategy",
        re: /\b(strategy|strategies)\b/i
    }, {
        intent: "screen",
        re: /\b(screen|candidate|find pool|search|research|token)\b/i
    }, {
        intent: "memory",
        re: /\b(memory|pool history|note|remember)\b/i
    }, {
        intent: "smartwallet",
...
function getToolsForRole(agentType, goal = "") {
    ...
    for (const { intent, re } of INTENT_PATTERNS) {
            if (re.test(goal)) {
                for (const t of INTENT_TOOLS[intent])
                    matched.add(t);
            }
        }
    ...
    function shouldRequireRealToolUse(goal, agentType, interactive = false) {
        if (agentType === "MANAGER")
            return false;
        if (DECISION_EXPLANATION_INTENTS.test(goal))
            return false;
        if (CONFIG_READ_ONLY_INTENTS.test(goal))
            return false;
        if (MUTATING_TOOL_INTENTS.test(goal))
            return true;
        ...
        for (let step = 0; step < maxSteps; step++) {
            ...
            try {
                ...
                if (msg.tool_calls) {
                    for (const tc of msg.tool_calls) {
                        if (tc.function?.arguments) {
                            try {
                                // Coba parse standar dulu
                                JSON.parse(tc.function.arguments);
                            } catch {
                                // Jika gagal, gunakan helper sanitize kita (termasuk Regex Magic)
                                const sanitized = sanitizeJSONArgs(tc.function.arguments);
                                
                                // Convert balik ke string karena API minta string, bukan object
                                tc.function.arguments = JSON.stringify(sanitized);
                                
                                log("warn", `Repaired history JSON args for ${tc.function.name}`);
                            }
                        }
                    }
                }
                ...
                const toolResults = await Promise.all(msg.tool_calls.map(async(toolCall) => {
                            ...
                            let functionArgs = sanitizeJSONArgs(toolCall.function.arguments);

                            if (Object.keys(functionArgs).length === 0 && toolCall.function.arguments && toolCall.function.arguments !== "{}") {
                                log("error", `Critical: Failed to parse/repair args for ${functionName}. Raw: ${toolCall.function.arguments}`);
                            }
